# Banana Shaker 要件定義書 v1.3

**更新日**: 2025-12-05  
**バージョン**: v1.3.0 参照画像・プレビュー機能強化
...
#### Image-to-Image (img2img) (**v1.3強化**)
- ✅ ベース画像アップロード機能
- ✅ **参照画像（Reference Images）の複数アップロード** 🆕
  - スタイルや内容のガイダンス用に複数画像を追加可能
  - グリッド表示で管理
  - 個別削除機能
- ✅ **クライアント側画像圧縮** 🆕
  - 最大1024pxにリサイズ
...
### v1.3.0 (2025-12-05)
- ✅ **参照画像（Reference Images）機能**
  - ベース画像に加え、複数の参照画像をアップロード可能
  - スタイル・コンテンツガイダンス用
  - グリッド表示・個別削除対応
- ✅ **クライアント側画像圧縮**
  - アップロード時に自動圧縮（最大1024px、JPEG 70%）
  - リクエストサイズの最適化
- ✅ **生成結果インラインプレビュー**
  - 生成後ギャラリーへリダイレクトせず、生成ページ内でプレビュー
  - 右パネルに生成画像をグリッド表示
- ✅ **プレビューからギャラリー遷移**
  - プレビュー画像クリックでギャラリーページへ移動
  - 履歴は自動的にギャラリーへ保存
- ✅ **Server Action最適化**
  - 生成レスポンスから画像データを分離（ID返却→別途取得）
  - レスポンスサイズ制限の回避
